...
public class ExcelExportServer extends ExcelExportBase {
     ...
     * 1000000
     */
    private static final int XSSF_MAXROW = 80;
    ...
    public void appendData(Workbook workbook, ExportParams entity, Class<?> pojoClass, Collection<?> dataSet, String[] exportFields, List<ExcelExportEntity> excelParams) {
        ...
        Sheet sheet = null;
        int lastRowNum = 0;
        boolean ifNewSheet = false;

        final int numberOfSheets = workbook.getNumberOfSheets();
        Drawing patriarch = null;
        //TODO getLastRowNum是否准确
        if(numberOfSheets==0 || workbook.getSheetAt(numberOfSheets-1).getLastRowNum()>=(XSSF_MAXROW-1)){
            ifNewSheet = true;
            //原来没有sheet
            try {
            ...
            lastRowNum = lastFlushedRowNum >0? lastFlushedRowNum+1 :0;
        }
        ...
        try {
            ...
            int index = 0;
            if(ifNewSheet==true){
                index = entity.isCreateHeadRows() ? createHeaderAndTitle(entity, sheet, workbook, excelParams) : 0;
            }else{
                index = lastRowNum;
            }
...
//            addStatisticsRow(getExcelExportStyler().getStyles(true, null), sheet);
            ...
            if (dataSet.size() > 0) {
                appendData(workbook, entity, pojoClass, dataSet,exportFields, excelParams);
            }
